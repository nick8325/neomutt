...
#define W (rd->envelope->state.cols - MaxHeaderWidth)
...
static void redraw_crypt_lines(struct ComposeRedrawData *rd)
{
  ...
  mutt_window_mvprintw(rd->envelope, HDR_CRYPT, 0, "%*s",
  ...
  mutt_window_clrtoeol(rd->envelope);
  mutt_window_move(rd->envelope, HDR_CRYPTINFO, 0);
  mutt_window_clrtoeol(rd->envelope);
  ...
  {
    mutt_curses_set_color(MT_COLOR_COMPOSE_HEADER);
    mutt_window_mvprintw(rd->envelope, HDR_CRYPTINFO, 40, "%s", _("Encrypt with: "));
    mutt_curses_set_color(MT_COLOR_NORMAL);
    mutt_window_printf("%s", NONULL(C_SmimeEncryptWith));
  }

#ifdef USE_AUTOCRYPT
  mutt_window_move(rd->envelope, HDR_AUTOCRYPT, 0);
  mutt_window_clrtoeol(rd->envelope);
  if (C_Autocrypt)
  {
    mutt_curses_set_color(MT_COLOR_COMPOSE_HEADER);
    ...
    mutt_curses_set_color(MT_COLOR_COMPOSE_HEADER);
    mutt_window_mvprintw(rd->envelope, HDR_AUTOCRYPT, 40, "%s",
...
static void redraw_mix_line(struct ListHead *chain, struct ComposeRedrawData *rd)
{
  ...
  mutt_curses_set_color(MT_COLOR_COMPOSE_HEADER);
  mutt_window_mvprintw(rd->envelope, HDR_MIX, 0, "%*s", HeaderPadding[HDR_MIX],
                       _(Prompts[HDR_MIX]));
  mutt_curses_set_color(MT_COLOR_NORMAL);
  ...
  STAILQ_FOREACH(np, chain, entries)
  {
    ...
    if (c + mutt_str_strlen(t) + 2 >= rd->envelope->state.cols)
      break;
...
static void draw_envelope_addr(int line, struct AddressList *al, struct ComposeRedrawData *rd)
{
  ...
  mutt_curses_set_color(MT_COLOR_COMPOSE_HEADER);
  mutt_window_mvprintw(rd->envelope, line, 0, "%*s", HeaderPadding[line],
                       _(Prompts[line]));
  mutt_curses_set_color(MT_COLOR_NORMAL);
  mutt_paddstr(W, buf);
...
static void draw_envelope(struct ComposeRedrawData *rd)
{
  ...
  mutt_curses_set_color(MT_COLOR_COMPOSE_HEADER);
  mutt_window_mvprintw(rd->envelope, HDR_SUBJECT, 0, "%*s",
  ...
  mutt_curses_set_color(MT_COLOR_COMPOSE_HEADER);
  mutt_window_mvprintw(rd->envelope, HDR_FCC, 0, "%*s", HeaderPadding[HDR_FCC],
                       _(Prompts[HDR_FCC]));
  mutt_curses_set_color(MT_COLOR_NORMAL);
  mutt_paddstr(W, fcc);
  ...
  mutt_window_mvaddstr(rd->abar, 0, 0, _("-- Attachments"));
  mutt_window_clrtoeol(rd->abar);
